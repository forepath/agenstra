...
In addition to the **`clients`** namespace used to proxy chat and tooling to each workspace’s agent-manager, the agent controller exposes a **`tickets`** Socket.IO namespace on the same port for ticket board and automation realtime. Autonomous ticket runs are driven by an in-process scheduler on the controller; see [Ticket automation](../features/ticket-automation.md), [WebSocket Communication](../features/websocket-communication.md), and [Tickets and Workspaces](../features/tickets-and-workspaces.md).
