...
- **GitHub Copilot** – `.github/copilot-instructions.md` (repository-wide rules), `.github/instructions/NAME.instructions.md` (path-specific instructions; `applyTo` is autogenerated from rule `globs`). Primary agents only (subagents are not supported by Copilot); skills are merged into instructions

Rules (`.mdc`), commands (`.command.mdc`), skills (`.skill.mdc`), and agents/subagents (`.agent.mdc`, `.subagent.mdc`) are all **MDC** (YAML frontmatter + body). They are keyed by **filename stem** (e.g. `refactor.command.mdc` → `refactor`). Tools that do not support separate skills (GitHub Copilot) receive skills merged into instructions automatically.
